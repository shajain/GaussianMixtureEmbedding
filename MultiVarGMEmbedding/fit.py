...
from MultiVarGMEmbedding.GaussianMixAE import GaussianMixAE as AutoEncoder
from NN.trainer import Trainer as Trainer
from MultiVarGMEmbedding.debug import Debug
...
class GMEFitting:
    ...
    def demo(cls):
        ...
        # dg.alpha = 0.5
        # x = dg.pn_data(n*2)[0]
        pos_dist = mvn(mean=np.zeros(2), cov=np.diag(np.ones(2)))
        neg_dist = mvn(mean=np.ones(2)*6, cov=np.diag(np.ones(2)))
        dg = DataGenerator(pos_dist, neg_dist, 1.0)
        #x = dg.pn_data(n*2)[0]
        x = dg.data_pos(n)[0]
        #x = np.vstack(x_p,x_n)
        pdb.set_trace()
